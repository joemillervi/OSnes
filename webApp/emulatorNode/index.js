...
var debug = require('debug')('crowdmu:worker');

if (!process.env.CROWDMU_ROM) {
  console.log('You must specify the ENV variable `CROWDMU_ROM` '
    + 'pointint to location of rom file to broadcast.');
  process.exit(1);
}

process.title = 'crowdmu-emulator';
...
// rom
var file = process.env.CROWDMU_ROM;
...
// save interval
var saveInterval = process.env.CROWDMU_SAVE_INTERVAL || 60000;
...
function load(){
  ...
  emu.on('frame', function(frame){
    io.emit('frame', frame);
    redis.set('crowdmu:frame', frame);
  });

  redis.get('crowdmu:state:' + hash, function(err, state){
    if (err) throw err;
    if (state) {
  ...
  function save(){
    ...
    setTimeout(function(){
      ...
      if (snap) {
        debug('saving state');
        redis.set('crowdmu:state:' + hash, msgpack.pack(snap));
        save();
      }
    }, saveInterval);
  }
...
sub.subscribe('crowdmu:move');
sub.on('message', function(channel, move){
  if ('crowdmu:move' != channel) return;
